# == Schema Information
#
# Table name: diagnostic_test_detail_fields
#
#  id                :integer          not null, primary key
#  diagnostic_test_detail_id :integer
...
class DiagnosticTestDetailField < ActiveRecord::Base
	belongs_to :study
	belongs_to :diagnostic_test_detail
	...
	def self.remove_old_choices obj_id, id_list
		choices = DiagnosticTestDetailField.where(:diagnostic_test_detail_id=>obj_id)
	...
	def self.save_question_choices(choices_hash, obj_id, editing, subquestion, options_with_subs,has_subquestion) 
		...
		choices_hash.keys.each do |key|
			...
			if editing
				...
				begin
					tmp = DiagnosticTestDetailField.find(field_id.to_i)
				rescue
					tmp = nil
				end
			end
			...
			if tmp.nil?
				...
				tmp = DiagnosticTestDetailField.new(:diagnostic_test_detail_id=>obj_id, :option_text=>choices_hash[key], :subquestion=>subquestion_text, :has_subquestion=>has_subquestion)
				if !tmp.save
					#print " Saving failed.\n\n"
					success = false
				else
