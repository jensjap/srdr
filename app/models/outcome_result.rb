...
class OutcomeResult < ActiveRecord::Base
			...
			for item in params
				...
				if (item[0].starts_with? "out") && !(item[0].ends_with? "calc")
					...
					for elem in item[1]
						...
						column_value = check_for_footnotes(column_value,field_name,study_id,"results")
    ...
    def self.check_for_footnotes(input,field_id,study_id, page_name)
    	retVal = input
      fid = field_id
      sid = study_id.to_i
    	...
    	if input =~ /\s*\d+\s*\[\s*\d?.*\]/
    		...
    		unless fnotes.empty?
    			...
    			previous_notes = FootnoteField.where(:study_id=>sid, :field_name=>fid)
    	  	...
    	  	fnotes.each do |fn|
    	  		#print " **** Creating a junction table entry.\n\nFootnote ID: #{fn}\nField_id: #{fid}\nStudy_ID: #{study_id}\n\n"
	    		  new_note = FootnoteField.new(:study_id=>sid,:field_name=>fid,:footnote_number=>fn.to_i, :page_name=>page_name)
	    		  new_note.save
	    		  if new_note.save
