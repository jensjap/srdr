...
namespace :remove_diagnostic_test_for_ef233 do
	task :update => :environment do
		# find comments that have a null post_type
		section_entry = ExtractionFormSection.find(:first, :conditions=>["extraction_form_id=? AND section_name=?",233,'diagnostics'])
